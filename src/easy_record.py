...
class InputThread(threading.Thread):
    ...
    def __init__(self, _t_start, _path_labels, _file_name):
        ...
        # Init labels file
        self.path_label_file = _path_labels + time.strftime("%Y-%m-%d-%H_%M_%S", time.gmtime(self.t_start)) \
                               + _file_name + '.csv'
        self.label_file = open(self.path_label_file, "w")
        self.label_file.write('Timestamp, PanNr, Content, Start/End [0,1]')
...
def record(_t_start, _file_name):
    # Starting Input Thread
    input_thread = InputThread(_t_start, path_labels, _file_name)
    input_thread.start()
...
while True:
    ...
    with picamera.PiCamera() as camera:
        ...
        record(t_start, file_name)
        camera.stop_recording()
        t_stop = time.time()
        print('The video has been saved as ' + file_name)
        print('The video is {:.1f} seconds long'.format(t_stop - t_start))
