...
int main() {
	...
	while (charCheckForEscKey != 27 && capWebcam.isOpened()) {		// until the Esc key is pressed or webcam connection is lost
		...
		cv::inRange(imgHSV, cv::Scalar(0, 155, 100), cv::Scalar(20, 255, 255), imgThreshLow);
		cv::inRange(imgHSV, cv::Scalar(165, 155, 155), cv::Scalar(179, 255, 255), imgThreshHigh);

		cv::Mat blackImage = cv::Mat::zeros(imgThreshLow.size(), imgThreshLow.type());

		cv::add(imgThreshLow, blackImage, imgThresh);
