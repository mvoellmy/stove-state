...
for file_name in file_names:
    for gesture_num in range(1,num_gestures+1):
        ...
        path_gesture = join(path_gestures, 'scegg', '{}'.format(gesture_num), file_name + '.h264')
        ...
        # out = cv2.VideoWriter('output{}.avi'.format(gesture_num), fourcc, 20.0, (background.shape[1], background.shape[0]), True)

        centroid_old = np.array([0,0])
        centroid_vel = np.array([0,0],dtype=np.float)
        # features_file = open(join(path_labels, "{}_".format(gesture_num) + file_name + "_features.csv"), "w")
        while (cap.isOpened()):
            ret, frame = cap.read()
            ...
            cv2.imshow("Segmented", segmented)  #segmented_final_color
