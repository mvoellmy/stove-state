...
def traverseTree(Tree, tree_depth, S, i, j):
    ...
    for k in range(0,tree_depth):
        ...
        if current_node.children != []:
            ...
            if F == G:
                idx_leaf = 2*idx_branch
                idx_leaf2 = 0
                #text_file.write("L\n")
            else:
                idx_leaf = 2*idx_branch + 1
                idx_leaf2 = 1
                #text_file.write("R\n")

    return idx_leaf, idx_branch, idx_leaf2
...
for t in range(0,num_trees):
    ...
    for i in range(pad,S.shape[0]-pad):
        for j in range(pad,S.shape[1]-pad):
            idx, idx_branch, idx_leaf2 = traverseTree(Tree, tree_depth, S, i, j)
            if S[i,j] == 0:
                leaf_values[idx,0] += 1
                Tree[idx_branch].data[4][idx_leaf2][0] += 1
            else:
                leaf_values[idx,1] += 1
                Tree[idx_branch].data[4][idx_leaf2][1] += 1
    
    #leaf_values[:,0] /= leaf_values[:,0].sum()
    #leaf_values[:,1] /= leaf_values[:,1].sum()
    #text_file.close()
    Forest.append((Tree, leaf_values))
...
for idx_tree, tree in enumerate(Forest):
    ...
    node_i = 0
    for idx, node in enumerate(tree[0]):
        print("Gamma: %s, Distrib: %s" % (node.data[2], Forest[0][1][idx]))
    for i in range(0,tree_depth):
        for j in range(0,2**i):
            text_file.write("%s \t" % (tree[0][node_i].data[2]))
            node_i += 1
        text_file.write("\n")
    text_file.write("\n")
...
for i in range(pad,S_n.shape[0]-pad):
    for j in range(pad,S_n.shape[1]-pad):   
        ...
        for idx, (tree, leaf_val) in enumerate(Forest):
            idx_leaf, idx_branch, idx_leaf2 = traverseTree(tree, tree_depth, S_n, i, j)
            probabilities[idx] = leaf_val[idx_leaf,:]        
